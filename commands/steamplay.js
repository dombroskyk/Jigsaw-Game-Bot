...
module.exports = {
    ...
    const inputToRegistrationMapping = inputUsers.map(inputUser => {
      const foundSteamUser = steamUsers.find(steamUser => steamUser.userId === inputUser.id)
    ...
    const user1SteamId = inputToRegistrationMapping.find(mapping => mapping.foundSteamUser.userId === inputUsers[0].id).foundSteamUser.steamId;
    ...
    const filteredFirstUserGameInfo = firstUserGameInfo.filter(gameInfo => typeof gameInfo !== "undefined");
    console.log(filteredFirstUserGameInfo);
    for (var gameInfo in filteredFirstUserGameInfo) {
      console.log(`${gameInfo.name}: ${gameInfo.tags?.join(",")}`);
    }
    // console.log(res2.data['70'].data);
	}
};
